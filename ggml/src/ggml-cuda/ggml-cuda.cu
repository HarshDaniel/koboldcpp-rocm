...
#ifndef NDEBUG
                // On integrated GPUs (APUs, e.g. RDNA3.5) the scheduler may
                // place an output in a host-visible buffer.  The compute path
                // supports that arrangement, as does the source-tensor check
                // below, so permit it here as well.
